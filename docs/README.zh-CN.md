...
- **使用熟悉的界面语言。** 插件会跟随 Obsidian，目前支持英文、简体中文、繁体中文、日文和韩文。
...
![加密 Markdown 文件的解密并打开按钮](images/decrypt-and-open.png)
...
![密码自动清除和 Vault 闲置自动锁定设置](images/session-security.png)

Ribbon 图标分别表达配置模式和密码状态：

![手动锁定、密码自动清除、闲置自动锁定和密码状态灯图例](images/security-mode-icons.png)

## 主要功能

- 从文件列表直接打开使用密码加密的 `.age` 文件。
- 解密 Markdown 和图片并将明文写回原目录，保留原生编辑、预览、链接、搜索和第三方插件能力。
- 关闭最后一个受保护明文 Tab 后，可选择加密刚关闭的文件、加密全部匹配明文或暂时保留明文。
- 提供 **立即加密并锁定 Vault** 命令和 Ribbon 锁按钮。
- 用户明确选择后，密码仅缓存在当前插件会话的内存中。
- Ribbon 锁按钮下方用红、绿、黄状态灯表示密码不可用、已缓存或计时器即将到期。
...
- 跟随 Obsidian 的界面语言，支持英文、简体中文、繁体中文、日文和韩文。

## 文件流程

打开 `notes/private.md.age` 并选择**解密并打开**：
...
选择**全部加密**或运行**立即加密并锁定 Vault**后，插件会主动清除当前会话缓存的密码；这不是 Obsidian 会话自然结束。之后再打开加密文件会重新询问密码。只加密刚关闭的单个文件时不会主动清除缓存。

插件不会依赖应用退出钩子进行交互式加密，因为 Obsidian 不会可靠地等待异步加密完成。退出前请先关闭受保护的 Tab，或运行**立即加密并锁定 Vault**。
...
设置页提供两个互斥的计时器，默认都为**关闭**：

- **自动清除密码**：从密码进入内存时开始固定计时。用户操作不会延长时间；到期只清除密码，不关闭 Tab，也不改变仍为明文的文件。
- **Vault 闲置后自动锁定**：当前 Vault 中发生键盘、鼠标、触摸、滚动、编辑或 Tab 切换时会重新计时。到期后，插件先等待 Markdown 编辑器保存，再加密所有匹配且未被排除的明文，关闭已经成功保护的 Tab，并清除密码，不再弹出确认框。

Ribbon 使用“钥匙＋小钟”表示自动清除密码，使用“锁＋环形箭头”表示自动闲置锁定；两个计时器都关闭时显示普通锁。右上角标记只表示配置模式，底部红、绿、黄状态灯分别表示密码不可用、可用和期限即将到期。设置页使用相同图标。

自动闲置锁定必须把密码保留在内存中，因此启用后密码窗口会固定开启**记住密码直到自动锁定**。Obsidian 重启后绝不会恢复密码；在用户再次输入密码前，自动锁定不会进入待命状态。

这里的闲置只表示插件没有在这个 Vault 的 Obsidian 主窗口或弹出窗口中观察到操作，不是操作系统级闲置检测。切换到其他应用后会继续计时；后台同步和文件修改时间不会重置计时。电脑休眠或 Obsidian 被暂停时，恢复后会立即重新检查期限。

## 加密范围设置和 `.ageconfig`

文件类型选项位于**设置 → Vault in Vault → 受保护的文件类型**，安全计时器也位于同一个设置页面。没有 `.ageconfig` 时，扩展名、排除路径和计时器设置保存在当前 Vault 插件目录的 `data.json` 中；密码永远不会写入其中。验证 Vault 密码后才能编辑保护策略，计时器则始终可以调整。
